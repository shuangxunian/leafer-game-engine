...
function getPackageEntrypoint(exportKey) {
  if (exportKey === ".") {
    return packageJson.name;
  }

  return `${packageJson.name}/${exportKey.slice(2)}`;
}

function normalizePackageTarget(path) {
  return path.startsWith("./") ? path.slice(2) : path;
}

function getPackageTargetUrl(target) {
  return new URL(`../${normalizePackageTarget(target)}`, import.meta.url);
}
...
test("root package metadata stays aligned with the root export", () => {
  assert.equal(packageJson.main, packageJson.exports["."].import);
  assert.equal(packageJson.types, packageJson.exports["."].types);
});

test("package export targets exist in the built artifact surface", async () => {
  for (const [entrypoint, config] of Object.entries(packageJson.exports)) {
    assert.equal(config.import.endsWith(".js"), true, `${entrypoint} import target should be JavaScript`);
    assert.equal(config.types.endsWith(".d.ts"), true, `${entrypoint} types target should be a declaration file`);

    await access(getPackageTargetUrl(config.import));
    await access(getPackageTargetUrl(config.types));
  }
});

test("public entrypoint docs track the package export map", async () => {
  ...
  for (const exportKey of Object.keys(packageJson.exports)) {
    const entrypoint = getPackageEntrypoint(exportKey);

    assert.equal(publicApi.includes(entrypoint), true, `public API docs should mention ${entrypoint}`);
    assert.equal(audit.includes(entrypoint), true, `API audit should mention ${entrypoint}`);
  }

  for (const nodeSafeEntrypoint of [
    `${packageJson.name}/core`,
    `${packageJson.name}/framework`,
    `${packageJson.name}/tooling`,
    `${packageJson.name}/adapter/render-types`
  ]) {
    assert.equal(audit.includes(nodeSafeEntrypoint), true);
    assert.equal(publicApi.includes(nodeSafeEntrypoint), true);
  }

  for (const browserFacingEntrypoint of [
    packageJson.name,
    `${packageJson.name}/adapter`,
    `${packageJson.name}/runtime`
  ]) {
    assert.equal(audit.includes(browserFacingEntrypoint), true);
    assert.equal(publicApi.includes(browserFacingEntrypoint), true);
  }
});
...
test("package verification script derives required docs from package files", async () => {
  ...
  assert.equal(verifyScript.includes("function collectRequiredPackageFiles()"), true);
  assert.equal(verifyScript.includes("for (const entry of packageJson.files)"), true);
  assert.equal(verifyScript.includes('if (path === "lib")'), true);
  assert.equal(verifyScript.includes("for (const requiredFile of collectRequiredPackageFiles())"), true);
  assert.equal(verifyScript.includes('"docs/version"'), true);
  assert.equal(verifyScript.includes("collectExportTargets()"), true);
  assert.equal(verifyScript.includes("assertRootMetadataMatchesExport()"), true);
});
...
test("dialogue choice example shell stays routed and package-facing", async () => {
  ...
  assert.equal(boot.includes("createSceneQuickStartBundle"), true);
...
test("quick-start game kit stage starts with scene input bridge bundle", async () => {
  ...
  assert.equal(readme.includes("当前项目已经推进到 `v1.0.0` Release Baseline"), true);
  ...
  assert.equal(collectBoot.includes("createSceneQuickStartBundle"), true);
  assert.equal(dialogueBoot.includes("createSceneQuickStartBundle"), true);
  assert.equal(pourBoot.includes("createSceneQuickStartBundle"), true);
  ...
  assert.equal(dodgeBoot.includes("createSceneQuickStartBundle"), true);
  assert.equal(dodgeBoot.includes("pointerButtons: true"), true);
  assert.equal(dodgeBoot.includes("quickStart.detach()"), true);
});
...
test("quick-start game kit stage adds scene quick-start bundle baseline", async () => {
  const roadmap = await readFile(new URL("../docs/roadmap.md", import.meta.url), "utf8");
  ...
  assert.equal(patch.includes("createSceneQuickStartBundle(scene, options?)"), true);
  assert.equal(patch.includes("does not add a project generator"), true);
  assert.equal(publicApi.includes("`v0.32.2` adds the scene quick-start bundle baseline"), true);
  assert.equal(publicApi.includes("createSceneQuickStartBundle(scene, options?)"), true);
  assert.equal(publicApi.includes("sequence `createSceneRuntimePreset(...)` before optional `createSceneInputBridgeBundle(...)`"), true);
  assert.equal(readme.includes("在 `v0.32.2` 补齐 scene quick-start bundle baseline"), true);
  assert.equal(readme.includes("createSceneQuickStartBundle(scene, options?)"), true);
  assert.equal(quickStartSource.includes("export function createSceneQuickStartBundle"), true);
  assert.equal(quickStartSource.includes("SceneQuickStartBundleOptions"), true);
  assert.equal(quickStartSource.includes("getRuntimePresetOptions"), true);
  assert.equal(quickStartSource.includes("createSceneRuntimePreset<TEvents>(scene"), true);
  assert.equal(quickStartSource.includes("createSceneInputBridgeBundle(scene, options.inputBridges)"), true);
  assert.equal(collectBoot.includes("createSceneQuickStartBundle"), true);
  assert.equal(collectBoot.includes("runtimeServices: true"), true);
  assert.equal(collectBoot.includes("inputBridges: { keyboard: true }"), true);
  assert.equal(dialogueBoot.includes("createSceneQuickStartBundle"), true);
  assert.equal(dialogueBoot.includes("runtime: { input: true }"), true);
  assert.equal(pourBoot.includes("createSceneQuickStartBundle"), true);
  assert.equal(pourBoot.includes("pointerButtons: { target }"), true);
  assert.equal(pourBoot.includes("pointerPosition: { target, localTarget: target }"), true);
  assert.equal(dodgeBoot.includes("createSceneQuickStartBundle"), true);
  assert.equal(dodgeBoot.includes("quickStart.detach()"), true);
  assert.equal(collectDocs.includes("createSceneQuickStartBundle(...)"), true);
...
test("quick-start game kit stage adds scene audio runtime bundle baseline", async () => {
  const roadmap = await readFile(new URL("../docs/roadmap.md", import.meta.url), "utf8");
  ...
  assert.equal(patch.includes("createSceneAudioRuntimeBundle(scene, options?)"), true);
  assert.equal(patch.includes("does not add an audio mixer"), true);
  assert.equal(publicApi.includes("`v0.32.4` adds the scene audio runtime bundle baseline"), true);
  assert.equal(publicApi.includes("createSceneAudioRuntimeBundle(scene, options?)"), true);
  assert.equal(publicApi.includes("does not add an audio mixer, audio editor"), true);
  assert.equal(readme.includes("在 `v0.32.4` 补齐 scene audio runtime bundle baseline"), true);
  assert.equal(readme.includes("createSceneAudioRuntimeBundle(scene, options?)"), true);
  assert.equal(audioSource.includes("export type SceneAudioRuntimeBundle"), true);
  assert.equal(audioSource.includes("export function createSceneAudioRuntimeBundle"), true);
  assert.equal(audioSource.includes("scene.getSystem(AudioRuntimeSystem) ?? addAudioRuntime(scene, options)"), true);
  assert.equal(dodgeScene.includes("createSceneAudioRuntimeBundle"), true);
  assert.equal(dodgeScene.includes("DODGE_BLOCKS_AUDIO_MANIFEST"), true);
  assert.equal(dodgeScene.includes("addAudioRuntime"), false);
  assert.equal(dodgeBoot.includes("BrowserAudioPlaybackAdapter"), true);
  assert.equal(dodgeBoot.includes("addAudioPlayback"), true);
  assert.equal(collectScene.includes("createSceneAudioRuntimeBundle"), false);
  assert.equal(pourScene.includes("createSceneAudioRuntimeBundle"), false);
  assert.equal(dialogueScene.includes("createSceneAudioRuntimeBundle"), false);
  assert.equal(dodgeDocs.includes("createSceneAudioRuntimeBundle(...)"), true);
  assert.equal(dodgeDocs.includes("不引入 Web Audio graph、mixer、音频编辑器或音频内容生产能力"), true);
});

test("quick-start game kit stage adds package-facing README example guide", async () => {
  const roadmap = await readFile(new URL("../docs/roadmap.md", import.meta.url), "utf8");
  ...
  assert.equal(patch.includes("docs/quick-start-game-kit.md"), true);
  assert.equal(patch.includes("does not add a project generator"), true);
  ...
  assert.equal(guide.includes("startSceneWithLifecycle"), true);
  assert.equal(guide.includes("createSceneQuickStartBundle"), true);
  assert.equal(guide.includes("createHudTextBundle"), true);
  assert.equal(guide.includes("createSceneAudioRuntimeBundle"), true);
  assert.equal(guide.includes("BrowserAudioPlaybackAdapter"), true);
  assert.equal(guide.includes("Game-specific rules stay downstream"), true);
...
test("stabilization stage hardens package artifact verification", async () => {
  const roadmap = await readFile(new URL("../docs/roadmap.md", import.meta.url), "utf8");
  ...
  assert.equal(patch.includes("collectRequiredPackageFiles()"), true);
  assert.equal(patch.includes("docs/version"), true);
  assert.equal(patch.includes("does not add new runtime API"), true);
  assert.equal(publicApi.includes("`v0.33.1` hardens package artifact verification"), true);
  assert.equal(publicApi.includes("derives required package-facing docs from `package.json.files`"), true);
  assert.equal(publicApi.includes("rejects `docs/version/` release-history files from the npm package"), true);
  assert.equal(readme.includes("在 `v0.33.1` 补齐 package artifact verification baseline"), true);
  assert.equal(audit.includes("`v0.33.1` hardens package artifact verification"), true);
  assert.equal(audit.includes("package.json.files"), true);
  assert.equal(audit.includes("docs/version/"), true);
  assert.equal(verifyScript.includes("collectRequiredPackageFiles()"), true);
  assert.equal(verifyScript.includes("for (const entry of packageJson.files)"), true);
  assert.equal(verifyScript.includes('"docs/version"'), true);
});

test("stabilization stage locks public entrypoint documentation", async () => {
  const roadmap = await readFile(new URL("../docs/roadmap.md", import.meta.url), "utf8");
  ...
  assert.equal(patch.includes("package export map"), true);
  assert.equal(patch.includes("does not add new runtime API"), true);
  assert.equal(publicApi.includes("`v0.33.2` locks public entrypoint documentation"), true);
  assert.equal(publicApi.includes("package export map"), true);
  assert.equal(readme.includes("当前项目已经推进到 `v1.0.0` Release Baseline"), true);
  assert.equal(readme.includes("在 `v0.33.2` 补齐 public entrypoint documentation audit baseline"), true);
  assert.equal(audit.includes("`v0.33.2` locks public entrypoint documentation"), true);
  assert.equal(audit.includes("package export map"), true);
});

test("stabilization stage aligns package metadata with public exports", async () => {
  const roadmap = await readFile(new URL("../docs/roadmap.md", import.meta.url), "utf8");
  ...
  assert.equal(patch.includes("package main/types metadata"), true);
  assert.equal(patch.includes("does not add new runtime API"), true);
  assert.equal(publicApi.includes("`v0.33.3` aligns package metadata with the public export map"), true);
  assert.equal(publicApi.includes("package `main` and `types` fields"), true);
  assert.equal(readme.includes("当前项目已经推进到 `v1.0.0` Release Baseline"), true);
  assert.equal(readme.includes("在 `v0.33.3` 补齐 package metadata export alignment baseline"), true);
  assert.equal(audit.includes("`v0.33.3` aligns package metadata with the public export map"), true);
  assert.equal(audit.includes("main` / `types`"), true);
  assert.equal(verifyScript.includes("assertRootMetadataMatchesExport()"), true);
});

test("one point zero release baseline stays package-facing", async () => {
  const roadmap = await readFile(new URL("../docs/roadmap.md", import.meta.url), "utf8");
  ...
  assert.equal(patch.includes("Release Baseline"), true);
  assert.equal(patch.includes("does not add new runtime API"), true);
  assert.equal(patch.includes("image-backed sprite rendering"), true);
  assert.equal(patch.includes("responsive browser runtime resize behavior"), true);
  assert.equal(patch.includes("pointer position, picking, selection, drag, and source-target helpers"), true);
  assert.equal(patch.includes("screen-space dialogue prompt and choice-flow primitives"), true);
  assert.equal(publicApi.includes("`v1.0.0` promotes the stabilized package/API boundary"), true);
  assert.equal(publicApi.includes("lightweight Web 2D game engine package baseline"), true);
  assert.equal(readme.includes("当前项目已经推进到 `v1.0.0` Release Baseline"), true);
  assert.equal(readme.includes("在 `v1.0.0` 完成 1.0 release baseline"), true);
  assert.equal(audit.includes("`v1.0.0` release baseline"), true);
  assert.equal(audit.includes("1.0 release shape"), true);
  assert.equal(audit.includes("major-version"), true);
});
...
test("framework package subpath can be imported by package name in Node", async () => {
  ...
  assert.equal(typeof framework.createSceneAudioRuntimeBundle, "function");
  ...
  assert.equal(typeof framework.createSceneQuickStartBundle, "function");
...
test("dodge-blocks example consumes pointer button input through semantic actions", async () => {
  ...
  assert.equal(boot.includes("quickStart.detach()"), true);
...
test("dodge-blocks example consumes opt-in browser audio playback APIs without editor scope", async () => {
  ...
  assert.equal(sceneSource.includes("createSceneAudioRuntimeBundle"), true);
  assert.equal(sceneSource.includes("addAudioRuntime"), false);
