...
`v0.31.5` adds no new public package API. It turns `examples/dialogue-choice` into a tiny playable dialogue flow with an example-owned prompt route table, keyboard focus, choice resolution, `DialoguePromptView.setPrompt(...)` updates, visited prompt tracking, last resolved choice tracking, an ending prompt, and a reset action. This proves existing dialogue data, choice state, prompt view, input action, keyboard bridge, GameFlow, and lifecycle helpers can support a small galgame-style prototype path while keeping prompt routing, choice effects, story content, ending conditions, and layout in the downstream example. It does not add a visual novel scripting language, branching story editor, dialogue timeline editor, story graph traversal engine, reusable story format, choice effect system, visual UI builder, launcher, gallery, marketplace, SDK wrapper, monetization, analytics, mobile app shell, or publishing workflow.
...
`v0.32.3` adds the HUD text bundle baseline. The `/framework` entrypoint now exports `createHudTextBundle(renderAdapter, renderScene, items)`, `HudTextBundle`, and `HudTextBundleItem` so downstream games can create several keyed screen-space text nodes through the existing `createHudText(...)` primitive without repeating one call per label. The helper returns keyed nodes, `get(id)`, and `destroy()` for the nodes it created, and it rejects duplicate ids before creating nodes. `examples/collect-stars` and `examples/dodge-blocks` consume it for repeated HUD node setup while keeping item ids, text, coordinates, font sizes, visibility, target layers, score/timer behavior, overlay choices, and gameplay snapshots example-owned. This is runtime/framework HUD assembly only; it does not add a HUD layout engine, theme system, widget library, UI builder, visual editor, launcher, gallery, template marketplace, SDK wrapper, account system, ads, monetization, analytics service, publishing workflow, game-specific rules engine, galgame script format, or content authoring workflow.

`v0.32.4` adds the scene audio runtime bundle baseline. The `/framework` entrypoint now exports `createSceneAudioRuntimeBundle(scene, options?)` and `SceneAudioRuntimeBundle` so downstream games can install or reuse the scene-owned `AudioRuntimeSystem` and receive its `AudioRuntimeState` through one small helper. The helper reuses existing audio runtime systems instead of adding duplicates, and passes `AudioRuntimeSystemOptions` only when it installs a new system. `examples/dodge-blocks` consumes it for scene-owned audio runtime setup while keeping the audio manifest, placeholder sources, semantic cue ids, cue trigger timing, browser playback adapter setup, tooling snapshot choices, and gameplay rules example-owned. This is runtime/framework audio setup only; it does not add an audio mixer, audio editor, waveform editor, playback buttons, volume sliders, asset browser, Web Audio graph builder, visual editor, launcher, gallery, template marketplace, SDK wrapper, account system, ads, monetization, analytics service, publishing workflow, game-specific rules engine, galgame script format, or content authoring workflow.
