...
`0.21.x` starts audio runtime primitives. The Node-safe `framework` entrypoint now exposes audio asset/cue/channel data contracts, `defineAudioManifest(...)`, and `AudioRuntimeState` / `createAudioRuntimeState(...)` so downstream games can describe audio intent, channel volume/mute state, and deterministic play/stop/pause/resume operation records without depending on Web Audio. This remains runtime contract work, not audio playback, decoding, mixer UI, waveform editing, or audio authoring.
...
- `framework` should stay usable for logic tests and reusable gameplay primitives, including input action mapping, pointer button action bindings, browser pointer button bridging, collision pair query snapshots, audio data contracts/runtime intent state, sprite animation timing helpers, component/system behavior, deterministic runtime event dispatch, update-driven scheduling, opt-in scene runtime service integration, data-driven scene config validation, safe scene config bootstrap, render/view synchronization, render-node capability checks, tile map data contracts, level spawn/region metadata, and optional scene config level/map declarations.
- `tooling` can expose structured snapshots and formatters in Node, including read-only system lifecycle state, sprite animation state, runtime services state, input action state, and collision pair state, but browser panel classes should only be constructed in a DOM environment.
