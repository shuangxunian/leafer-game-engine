...
- `examples/dodge-blocks`、`examples/collect-stars`、`examples/pour-sort` 和 `examples/dialogue-choice` 作为下游消费者样例，用来验证引擎分层、异步资源加载、sprite animation、音频、输入、碰撞、相机基础、HUD、tile visual layer、actor template、可玩小游戏能力、pointer-first puzzle loop 以及 dialogue choice shell；`0.27.x` 已经补齐 pointer position、命中测试、entity picking 和 source-target selection state 这类 pointer-first 小游戏会用到的基础能力。
